...
const { setupAuth, registerAuthRoutes } = require('./supabase_auth');
...
app.use('/api/fire', verifyToken, require('./routes/fire'));
app.use('/api/liabilities', verifyToken, require('./routes/liabilities'));

app.get('/', (req, res) => res.sendFile(path.join(__dirname, '../code/index.html')));
app.get('/index.html', (req, res) => res.sendFile(path.join(__dirname, '../code/index.html')));

if (require.main === module) {
  initDb()
    .then(() => app.listen(PORT, () => console.log(`WealthOS server running at http://localhost:${PORT}`)))
